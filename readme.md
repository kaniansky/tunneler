...
![Blue player view](screenshots/blue.png)
![Green player view](screenshots/green.png)

**Spectator view** — both halves, plus a live map overview and scoreboard:

![Spectator view](screenshots/spectate.png)

**Local split screen** (`/play`) — one browser, hotseat, both tanks at once:

![Local split screen](screenshots/split-screen.png)

**Singleplayer vs AI** (`/ai/easy`, `/ai/medium`, `/ai/hard`) — blue is you, green is a built-in bot,
full split screen like `/play` since there's no second human to hide the other half from:
...
(`http://localhost:8042/play` for local same-browser split screen, no session needed, or
`http://localhost:8042/ai/easy`/`/ai/medium`/`/ai/hard` to play blue against the built-in AI instead of a
second human). The https listener
...
- game (source for all browser-served assets - see "Building / development" above for how this reaches the browser)
  ...
  - **ai.js** / **ai-combat.js** / **ai-pathing.js** / **ai-sighting.js** / **ai-debug.js** - built-in bot
    ...
    `AiController` - see `ai.js`'s header comment for the full breakdown
